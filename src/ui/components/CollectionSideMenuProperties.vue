...
const collectionViewType = collectionViewRecordValue.getType();

const visiblePropertyIds = collectionViewRecordValue.getProperties().map(o => o.property);
    ...
    if (visibile === true) {
        for(let i = 0; i<hiddenProperties.value.length; i++) {
            if (hiddenProperties.value[i].id === id) {
                ...
                collectionViewRecordValue.addProperty(property.id);
                break;
            }
        }
    }
        ...
        for(let i = 0; i<visibleProperties.value.length; i++) {
            if (visibleProperties.value[i].id === id) {
                ...
                collectionViewRecordValue.removePropertyById(property[0].id);
                break;
            }
        }
    }
